      ...
      <el-form :inline="true" :model="filters" :size="size">
        ...
        <el-form-item>
          <kt-button :label="$t('action.refresh')" icon="fa fa-refresh"
                     perms="sys:log:view" @click="findPage(null)"></kt-button>
        </el-form-item>
      </el-form>
    </div>
    <!--表格内容栏-->
    <kt-table :columns="columns" :data="pageResult" permsDelete="sys:log:delete"
...
export default {
  ...
  methods: {
    ...
    handleDelete: function (data) {
      this.$api.log.Delete({'logId': data.params}).then(data.callback)
    },
    // 时间格式化
