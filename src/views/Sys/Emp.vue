    ...
    <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="dataForm" :size="size"
        ...
        <el-form-item label="职工号" prop="empNo">
          <el-input v-model="dataForm.empNo" auto-complete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleDisable: function (data) {
      this.$api.emp.batchDisable(data.params).then(data.callback)
    },
    //批量恢复
    handleRecover: function (data) {
      this.$api.emp.batchRecover(data.params).then(data.callback)
    },
    // 显示新增界面
    handleAdd: function () {
